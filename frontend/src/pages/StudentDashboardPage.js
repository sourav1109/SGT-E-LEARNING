...
import { Menu as MenuIcon, Home as HomeIcon, VideoLibrary as VideoLibraryIcon, History as HistoryIcon, ExitToApp as LogoutIcon, AccountCircle as AccountIcon, Notifications as NotificationsIcon } from '@mui/icons-material';
...
const StudentDashboardPage = () => {
  ...
  const [blink, setBlink] = useState(false); // transient blink on increase
  const [annBlink, setAnnBlink] = useState(false); // persistent blink while unread announcement exists
  ...
  useEffect(() => {
    ...
    const fetchUnread = async () => {
      try {
        ...
        setPrevUnread(count);
        if (count > 0) {
          // fetch limited list to check for announcement
  ...
  const openNotifications = async (e) => {
    ...
    try {
      ...
      if (unreadCount > 0) {
        ...
        setUnreadCount(0);
        setAnnBlink(false); // stop persistent announcement blink once panel opened and marked read
      }
    } catch (e) {/* ignore */}
  };
